...
本文对照仓库已生成的图表，说明**怎么读结果、怎么讲故事、Isaac 演示怎么跑**。  
适合自学复盘与面试展示。
...
> **算法与接口与 Isaac 路径相同**，结论结构可直接用于展示；面试时需诚实说明数据来源。
...
1. **方法自洽（基准组）** — 理想数据上误差 ≈ 0，证明 \(τ=Yπ\)、QR、求解没写错。  
2. **接近真机（工程组）** — 摩擦 + 噪声 + 异常点后，普通 OLS 变差。  
3. **工程价值（鲁棒组）** — 同一份脏数据上，robust WLS **压低内点误差**；再接重力补偿闭环。
...
→ 展示时强调：**内点 0.24 → 0.05（约 5×）**，这才是 robust 的意义。
